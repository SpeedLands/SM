...
try:
    ...
    for row in sheet.iter_rows(min_row=2, values_only=True):
        ...
        if email and password:
            ...
            # FIX: auto-correct .mc typo
            if clean_email.endswith('.mc'):
                clean_email = clean_email[:-3] + '.mx'
                print(f"Corrected typo for: {clean_email}")
                
            excel_passwords[clean_email] = str(password).strip()
            row_count += 1
