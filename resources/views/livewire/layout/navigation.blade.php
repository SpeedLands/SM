    ...
    <flux:navlist.group heading="{{ $isViewParent ? 'Mis Hijos' : 'Plataforma' }}" class="grid">
        ...
        @if($isViewStaff)
            @can('admin-only')
                ...
                <flux:navlist.item href="{{ route('data-exporter') }}" icon="document-arrow-down" :current="request()->routeIs('data-exporter')">Exportar Datos</flux:navlist.item>
        ...
        <flux:navlist.item data-tour="nav-tutorials" icon="question-mark-circle" href="{{ route('tutorials.index') }}" :current="request()->routeIs('tutorials.index')">Tutoriales</flux:navlist.item>
    </flux:navlist.group>
</flux:navlist>
