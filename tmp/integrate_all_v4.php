...
for ($row = 2; $row <= $highestRowOrig; $row++) {
    ...
    // Look for group pattern (e.g. 1A, 2G, 3I) in cols 4, 5, 6
    $group = null;
    $turno = null;
    $address = null;
    ...
    $origRecords[] = [
        ...
        'group' => $group,
        'turno' => $turno ?: 'MATUTINO', // Default if not found
        'address' => $address,
        'parts' => explode(' ', $name),
    ];
}

// 3. Match CURP DATA to ORIGINAL
...
for ($row = 2; $row <= $highestRowCurp; $row++) {
    ...
    // Find matching record in ORIGINAL
    $match = null;
    ...
    if ($match) {
        ...
        echo "Warning: Could not match CURP $curp (".implode(' ', $parts).") to any record in ORIGINAL.xlsx\n";
    }
}
...
{
    return strtolower(preg_replace('/[^a-z0-9]/i', '', iconv('UTF-8', 'ASCII//TRANSLIT', $text)));
}
...
foreach ($finalIntegrationList as $s) {
    ...
    $file = "$dir/$group.xlsx";
    if (! file_exists($file)) {
        echo "Error: File $file not found for group $group. Skipping.\n";

        continue;
    }

    echo "Processing {$s['full_name']} into $file...\n";

    $tmpFile = "tmp/{$group}_final.xlsx";
    copy($file, $tmpFile);
    ...
    if ($alSheet && $pdSheet) {
        ...
        $rowAl = [$s['full_name'], $s['turno'], $group, $s['address'], null, "{$s['item1']} / {$s['item2']}", $s['curp']];
        ...
        echo "  Appended student and parent ($slug).\n";
